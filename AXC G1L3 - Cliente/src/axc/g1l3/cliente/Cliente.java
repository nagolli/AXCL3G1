...
class Cliente {
   ...
   private static Socket cliente = new Socket();
  ...
  public void Conectar() 
    {
        try
        {
            ...
            BufferedReader recibir_datos = new BufferedReader(new InputStreamReader (cliente.getInputStream()));
            
            mensaje_enviado = recibir_datos.readLine();
            enviar_datos.writeByte(Integer.parseInt(mensaje_enviado));
            System.out.println("FROM SERVER " + enviar_datos);
   ...
   // Envia al Servidor el número de grupo seleccionado
    ...
    public void enviarUbicacion() throws SocketException, UnknownHostException, IOException
    {
        ...
        try
        {
           ...
           paquete = new DatagramPacket (mensaje_bytes, message.length(), address,PUERTO);
           UDP.send(paquete);
